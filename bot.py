...
from discord import GatewayNotFound, HTTPException, app_commands
...
from extras.viviatools import config, personality_message, server_config
...
if __name__ != "__main__":
    print(
        constants.__NOTMAIN__, sys.stderr
    )
    print(
        "Psst! If you're looking for helper functions, you probably want ViviaTools."
    )
    sys.exit(0)
...
argparser.add_argument(
    "--debug", action="store_true", help="Enable debug mode for this session"
)
argparser.add_argument(
    "--beta", action="store_true", help="Enable loading ViviaBase-beta extensions"
)
argparser.add_argument(
    "--disable-sharded",
    action="store_true",
    help="Disable sharded mode for this session",
)
...
# LLaMa
# TODO: this is slow, run it during setup_hook
...
if sharded:
    bot = commands.AutoShardedBot(
        command_prefix=config["General"]["Prefix"], intents=intents
    )
else:
    viviatools.log(
        "Sharded mode is disabled - running in non-sharded mode, performance may be degraded",
        logging.WARNING,
    )
        ...
        for root, _, files in os.walk("commands"):
            for file in files:
                if file.endswith(".vse"):
                    futures.append(
                        executor.submit(viviatools.extract_vse, os.path.join(root, file))
                    )
        for future in concurrent.futures.as_completed(futures):
            if future.exception():
                viviatools.log(
                    f"Failed to extract VSE extension {future.result()[1]}",
                    logging.ERROR,
                )
                viviatools.log(
                    "".join(traceback.format_exception(future.exception())),
                    logging.ERROR,
                )
                viviatools.log(
                    "VSE extension will not be loaded - functionality may be limited.",
                    logging.ERROR,
                )
                if debug:
                ...
                viviatools.log(
                    f"VSE extension {future.result()[1]} extracted", logging.DEBUG
                )
...
async def on_error(event, *args, **kwargs):
    ...
    try:
        await args[0].send(personality_message("errors.error"))
    except Exception:
...
async def on_app_command_error(interaction, error: app_commands.AppCommandError):
    ...
    viviatools.log(
        f"""App command error in interaction {interaction}! \
        {''.join(traceback.format_exception(*sys.exc_info()))}""",
        logging.ERROR,
    )
    match type(error):
        case app_commands.CommandSignatureMismatch:
            ...
            await interaction.response.send_message(
                personality_message("errors.commandoutofsync")
            )
        case app_commands.CommandNotFound:
            # Command not found
            await interaction.response.send_message(
                personality_message("errors.commandnotfound")
            )
        case _:
            # Anything else
            await interaction.response.send_message(personality_message("errors.error"))
...
async def on_command_error(ctx: commands.Context, error: CommandError):
    ...
    match errtype:
        case errors.CommandNotFound:
            ...
            await ctx.send(personality_message("errors.commandnotfound"))
        case errors.MissingRequiredArgument:
            # Missing argument
            viviatools.log(
                f"Missing required argument(s) in 'v!{ctx.invoked_with}': {error.param.name}",
                logging.WARNING,
            )
            viviatools.log("".join(traceback.format_exception(error)), logging.WARNING)
            await ctx.send(
                personality_message("errors.missingarguments").replace(
        ...
        case errors.BadArgument:
            # Invalid argument
            viviatools.log(
                f"Bad argument(s) in 'v!{ctx.invoked_with}': {error.param.name}",
                logging.WARNING,
            )
            viviatools.log("".join(traceback.format_exception(error)), logging.WARNING)
            await ctx.send(
                personality_message("errors.badarguments").replace(
                    "{arg}", error.param.name
                )
            )
        case errors.BotMissingPermissions | errors.MissingPermissions:
            ...
            viviatools.log(
                f"Missing permissions in 'v!{ctx.invoked_with}': {error.missing_permissions}",
                logging.WARNING,
            )
            viviatools.log("".join(traceback.format_exception(error)), logging.WARNING)
            await ctx.send(
                personality_message("errors.missingpermissions").replace(
        ...
        case errors.CommandInvokeError:
            ...
            viviatools.log(
                f"Command invoke error in 'v!{ctx.invoked_with}': {error}",
                logging.WARNING,
            )
            viviatools.log("".join(traceback.format_exception(error)), logging.WARNING)
            await ctx.send(personality_message("errors.error"))
        case _:
            # Anything else
            viviatools.log(
                f"An unhandled error occurred in 'v!{ctx.invoked_with}': "
...
async def on_ready():
    ...
    await viviatools.set_custom_presence("POWERING UP - Loading extensions...", bot)
    await reload_all_extensions()

    # Statuses
    try:
        status_changes.start()
    except RuntimeError:
...
async def status_changes():
    ...
    for file in os.listdir("data/statuses"):
        if file.endswith(".json"):
            with open(
                os.path.join("data", "statuses", file), "r", encoding="utf-8"
            ) as f:
                statuses.extend(json.load(f)["statuses"])
    status = random.choice(statuses)
    await viviatools.set_custom_presence(status, bot)
...
async def on_guild_join(guild: discord.Guild):
    ...
    def setup_guild_data():
        if debug:
            viviatools.log(
                f"Setting up custom quotes, config file, and Vivia admin role for {guild.name} ({guild.id})",
                logging.DEBUG,
            )
        with open(
            os.path.join("data", "servers", str(guild.id), "quotes.json"),
            "w",
            encoding="utf-8",
        ) as f:
            json.dump({"quotes": []}, f)
        with (
            open(
                os.path.join("data", "servers", str(guild.id), "config.json"),
                "w",
                encoding="utf-8",
            ) as f,
            open(
                os.path.join("data", "config.json.example"), "r", encoding="utf-8"
            ) as g,
        ):
        ...
        for member in guild.members:
            if member.guild_permissions.administrator:
                role = discord.utils.find(
                    lambda r: r.name == "Vivia Admin", guild.roles
                )
                if role is None:
                    role = asyncio.run_coroutine_threadsafe(
                        guild.create_role(
                            name="Vivia Admin",
                            reason="Vivia setup: Users with this role have privileges when running Vivia's commands in this server.",
                        ),
                        bot.loop,
                    ).result()
                asyncio.run_coroutine_threadsafe(
                    member.add_roles(
                        role,
                        reason="Vivia setup: This user has administrative permissions and was automatically assigned to the Vivia Admin role.",
                    ),
                    bot.loop,
                )
                viviatools.log(
                    f"User {member.name} ({member.id}) was automatically assigned the Vivia Admin role in {guild.name} ({guild.id}).",
                    logging.DEBUG,
                )
...
async def on_message(message: discord.Message):
    ...
    # Guilds
    if server_config(message.guild.id)["aiEnabled"]:
        if message.mentions and message.mentions[0] == bot.user:
            ...
            thread = threading.Thread(
                ...
                args=(
                    (
                        message.content.removeprefix(f"<@{str(message.author.id)}>"),
                        message.author.display_name,
                        message.author.name,
                        message.channel,
                        bot.loop,
...
async def reload_all_extensions():
    ...
    unload_threads = [
        threading.Thread(
            target=asyncio.run, args=(viviatools.unload_extension(extension),)
        )
    ...
    for extension in os.listdir(os.path.join("commands", "viviabase")):
        if extension.endswith(".py"):
            load_tasks.append(
                threading.Thread(
                    target=asyncio.run,
                    args=(viviatools.load_extension(extension, "commands.viviabase"),),
                )
            )
            ...
            if extension.endswith(".py"):
                load_tasks.append(
                    threading.Thread(
                        target=asyncio.run,
                        args=(
                            viviatools.load_extension(
                                extension, "commands.viviabase-beta"
                            ),
                        ),
                    )
                )
    ...
    for extension in os.listdir(os.path.join("commands")):
        if extension.endswith(".py"):
            load_tasks.append(
                threading.Thread(
                    target=asyncio.run,
                    args=(viviatools.load_extension(extension, "commands"),),
                )
            )
    ...
    viviatools.log(
        f"Loaded {len(viviatools.loaded_extensions)} extensions - failed loading {len(viviatools.failed_extensions)}."
    )
...
async def fixconfig(ctx: commands.Context):
    ...
    viviatools.log("Regenerating missing data files for all servers...", logging.INFO)
    threads = []
    ...
    await ctx.send(
        "Fixed all missing config and quotes files. Check log for more info."
    )
...
def regen_server_files(guild):
    """
        Regenerates missing data files for a server.
    """
    ...
    if not os.path.exists(os.path.join("data", "servers", str(guild.id))):
        ...
        viviatools.log(
            f"Data path for {guild.name} ({guild.id}) was regenerated.", logging.INFO
        )
        ...
        try:
            with (
                open(
                    os.path.join("data", "servers", str(guild.id), "config.json"), "x", encoding="utf-8"
                ) as f,
                open(os.path.join("data", "config.json.example"), "r", encoding="utf-8") as g,
            ):
                json.dump(obj=json.load(g), fp=f)
            viviatools.log(
                f"Config file for {guild.name} ({guild.id}) was regenerated.",
                logging.INFO,
            )
        ...
        try:
            with open(
                os.path.join("data", "servers", str(guild.id), "quotes.json"), "x", encoding="utf-8"
            ) as f:
                json.dump({"quotes": []}, f)
            viviatools.log(
                f"Custom quote file for {guild.name} ({guild.id}) was regenerated.",
                logging.INFO,
            )
        except FileExistsError:
            pass  # Most likely there was nothing wrong with it
        ...
        try:
            with open(
                os.path.join("data", "servers", str(guild.id), "warns.json"), "x", encoding="utf-8"
            ) as f:
                json.dump({"warns": []}, f)
            viviatools.log(
                f"Warn file for {guild.name} ({guild.id}) was regenerated.",
                logging.INFO,
            )
        except FileExistsError:
            pass  # Most likely there was nothing wrong with it
...
async def statuschange(ctx: commands.Context):
    ...
    await status_changes()
    await ctx.send("Status randomized!")


# TODO: see viviallama:27
@bot.hybrid_command(
    name="clearhistory", description="Clears your recent chat history with me."
)
async def clearhistory(ctx: commands.Context):
    ...
    if os.path.exists(os.path.join("data", "tempchats", str(ctx.author.name))):
        ...
        await ctx.send(personality_message("ai.historyclear"), ephemeral=True)
        viviatools.log(f"{ctx.author.name} cleared their chat history", logging.DEBUG)
    else:
        await ctx.send(personality_message("ai.nohistory"), ephemeral=True)


@bot.hybrid_command(name="setting", description="Manages Vivia's configuration.")
...
async def setting(ctx: commands.Context, option: str, value: bool):
    ...
    try:
        match (option):
            case "aiEnabled":
                changed = server_config(ctx.guild.id)
                changed["aiEnabled"] = value
                with open(f"data/servers/{ctx.guild.id}/config.json", "w", encoding="utf-8") as f:
                    json.dump(changed, f)
            case "verboseErrors":
                changed = server_config(ctx.guild.id)
        ...
        await ctx.send(personality_message("errors.error"), ephemeral=True)
        if server_config(ctx.guild.id)["verboseErrors"]:
            await ctx.send(
                f"{str(type(e))}: {e}\n-# To disable these messages, run /config verboseErrors false"
            )
        viviatools.log(
            f"Error while changing config for {ctx.guild.name} ({str(ctx.guild.id)}): {str(type(e))}: {str(e)}",
            severity=logging.ERROR,
        )


@bot.hybrid_command(name="reboot", description="Performs a full reboot of Vivia.")
@viviatools.owner_only
async def reboot(ctx: commands.Context, pull_git: bool = False):
    ...
    ## Notes:
        - Because this command replaces the running Vivia script with another one, any changes
          you've made to Vivia will take effect after this command is run.
        - `pull_git` requires `git` to be installed on your system, but you probably already have it
          if you're running Vivia anyway, don't you?
        - `pull_git` will also run `pip install -r requirements.txt` in the root to
          automatically install any new dependencies.
        - `pull_git` will OVERRIDE LOCAL CHANGES TO VIVIA! Be careful!
          (This does not affect custom extensions.)
    """

    await ctx.send("Rebooting...")
    viviatools.log(
        f"Rebooting on request of {ctx.author.name} ({str(ctx.author.id)})..."
    )
    await bot.close()
    if pull_git:
        try:
            ...
            viviatools.log(
                f"Failed to pull git repository: {str(type(e))}: {str(e)}",
                logging.ERROR,
            )
...
@bot.hybrid_command(
    name="listextensions", description="Displays Vivia's available extensions."
)
async def listextensions(ctx: commands.Context):
    ...
    await ctx.send(
        (
            "Available extensions: \n- " + ("\n- ".join(viviatools.loaded_extensions))
            if len(viviatools.loaded_extensions) > 0
            else "No extensions loaded? Wait, what?!"
        ),
        ephemeral=True,
    )
    await ctx.send(
        (
            ...
            + ("\n- ".join(viviatools.failed_extensions))
            if len(viviatools.failed_extensions) > 0
            else "No extensions failed to load!"
        ),
        ephemeral=True,
    )


@bot.hybrid_command(name="version", description="Displays Vivia's version.")
async def version(ctx: commands.Context):
    """
    Displays Vivia's version.
    """
    await ctx.send(
        personality_message("base.version").replace("{version}", __VERSION__),
        ephemeral=True,
    )
...
while True:
    try:
        bot.activity = discord.CustomActivity(
            name="POWERING UP - Connecting to Discord..."
        )
        bot.run(token, log_handler=None)
    except TypeError:
        viviatools.log(
            "Unable to start Vivia. Is the token in token.env correct?", logging.ERROR
        )
        viviatools.log(
            "If token.env doesn't exist, create it and place your bot token inside.",
            logging.ERROR,
        )
        sys.exit(1)
        ...
        if "Session is closed" in str(e):
            ...
            sys.exit(0)
    except (ClientConnectorError, GatewayNotFound, HTTPException):
        viviatools.log(
            "Vivia can't connect to Discord. Is your internet connection working, or is Discord's API down?",
            logging.ERROR,
        )
        viviatools.log(
            "Perhaps the token in token.env is invalid? There's a lot of reasons this could happen.",
            logging.ERROR,
        )
        ...
        )  # Using bot.run again will just give a "Session is closed" error and crash the script
    except Exception as e:
        # Unhandled exception
        ...
        viviatools.log(
            "I would appreciate if you would report this on GitHub, please.",
            logging.FATAL,
        )
        asyncio.run(
