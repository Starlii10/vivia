...
from extras.viviatools import log, personality_message, config
...
async def namegenerator(
    ctx: commands.Context, type: str = "first", gender: str = "none"
):
    """
    Generator for names.
    """
    with open(os.path.join("data", "names.json"), "r", encoding="utf-8") as f:
        names = json.load(f)
        ...
        match type:
            case "first":
                match gender:
                    case "male":
                        name = names["first"]["male"][
                            random.randint(0, len(names["first"]["male"]) - 1)
                        ]
                    case "female":
                        name = names["first"]["female"][
                            random.randint(0, len(names["first"]["female"]) - 1)
                        ]
                    case _:
                ...
                match gender:
                    case "male":
                        name = (
                            ...
                            + " "
                            + names["middle"][
                                random.randint(0, len(names["middle"]) - 1)
                            ]
                            + " "
                            + names["last"][random.randint(0, len(names["last"]) - 1)]
                        )
                    case "female":
                        name = (
                            names["first"]["female"][
                    ...
                    case _:
                        name = (
                            all_names[random.randint(0, len(all_names) - 1)]
                            + " "
                            + names["middle"][
                                random.randint(0, len(names["middle"]) - 1)
                            ]
    ...
    await ctx.send(personality_message("base.namegeneration").replace("{name}", name))
