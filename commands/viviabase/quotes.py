...
async def addquote(ctx: commands.Context, quote: str, author: str, date: str):
    ...
    try:
        ...
        await log(
            f'Failed to add "{quote} - {author}, {date}" to the custom quote list for server {ctx.guild.name} ({ctx.guild.id}): {type(e)}: {e}',
            severity=logging.ERROR,
        )
        return
    await ctx.send(f'"{quote}" - {author}, {date} was added to the list.')
    await log(
        f'{ctx.author} added "{quote} - {author}, {date}" to the custom quote list for server {ctx.guild.name} ({ctx.guild.id})',
        severity=logging.DEBUG,
    )


@commands.hybrid_command(
    name="quote", description="Sends a random (slightly insane) quote."
)
async def quote(ctx: commands.Context):
    ...
    try:
        with open(os.path.join("data", "quotes.json"), "r", encoding="utf-8") as f:
            if ctx.guild:
                with open(
                    os.path.join("data", "servers", str(ctx.guild.id), "quotes.json"),
                    "r",
                    encoding="utf-8",
                ) as g:
        ...
        await log(
            f"Couldn't send a quote for server {ctx.guild.name} ({ctx.guild.id}): {type(e)}: {e}",
            severity=logging.ERROR,
        )


@commands.hybrid_command(name="listquotes", description="Sends a list of all quotes.")
@app_commands.describe(customonly="Whether to only list custom quotes.")
async def listquotes(ctx: commands.Context, customonly: bool = False):
    ...
    try:
        if customonly:
            if ctx.guild:
                with open(f"data/servers/{ctx.guild.id}/quotes.json", "r", encoding="utf-8") as f:
                    quotes = json.load(f)["quotes"]
                    quotes = "\n".join(quotes)
                    await ctx.send(quotes)
            else:
                await ctx.send(
                    personality_message("errors.error")
            ...
            with open("data/quotes.json", "r", encoding="utf-8") as f:
                if ctx.guild:
                    with open(f"data/servers/{ctx.guild.id}/quotes.json", "r", encoding="utf-8") as g:
                        default_quotes = json.load(f)
                        custom_quotes = json.load(g)
                        quotes = default_quotes["quotes"] + custom_quotes["quotes"]
                        quotes = "\n".join(quotes)
        ...
        await log(
            f"Couldn't list quotes for server {ctx.guild.name} ({ctx.guild.id}): {type(e)}: {e}",
            severity=logging.ERROR,
        )
...
@app_commands.describe(quote="The quote to remove.")
@viviatools.block_in_dms
@viviatools.admin_only
async def removequote(ctx: commands.Context, quote: str):
    ...
    try:
        ...
        await log(
            f'Failed to remove "{quote}" from the list for server {ctx.guild.name} ({ctx.guild.id}): {type(e)}: {e}',
            severity=logging.ERROR,
        )
        return
    await ctx.send(f'"{quote}" was removed from the list.')
    await log(
        f'{ctx.author} removed "{quote}" from the list for server {ctx.guild.name} ({ctx.guild.id})',
        severity=logging.DEBUG,
    )
...
@viviatools.block_in_dms
@viviatools.admin_only
async def contextmenu_addquote(
    interaction: discord.Interaction, message: discord.Message
):
    """
    The context menu command to add a custom quote.
    Equivalent to the `addquote` command for a specific message.

    ## Args:
        - message (discord.Message): The message to add as a custom quote (will always be the message that was right-clicked).
        - interaction (discord.Interaction): The interaction that triggered the command.
    """
    add_custom_quote(
        f"\"{message.content}\" - {message.author.display_name}, {message.created_at.strftime('%Y-%m-%d')}",
        interaction.guild.id,
    )
    await interaction.response.send_message(
        f"\"{message.content}\" - {message.author.display_name}, {message.created_at.strftime('%Y-%m-%d')} was added to the list.",
        ephemeral=True,
    )
